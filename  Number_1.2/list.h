...
struct point{//есть класс точки-структура из двух дабл(x и y)
  double x;
  double y;
  point ()
  {
  ...
  {
    x = x_;//все приватные члены с подчёркиванием
    y = y_;
  }
  point (point const &p)
  {
...
bool operator == (const point &p1, const point &p2);//сравнения
double length (const point &p1);

////////////////////////////////////////////////
////////////////////////////////////////////////

struct list_node//элемент листа(у него 2 поля-поле точки и ссылка на следующий элемент)
{
  point p;//операторы всякие для них реализовал-равно, делить равно, конструктор присваивания, копирования, перемещения
  list_node *next;
  list_node ()
  {
    next = nullptr;
...
class list//класс листа-глава, конец, взять следующий, распечатать весь список, обновить список, удаление функции
{
private:
  list_node *head;//глава
  list_node *tail;//конец
  int size;
public:
  list ()
  {
